...
export function buildWeeklyReportMessages({ facts, promptVersion, model }: { facts: unknown; promptVersion: 'v0' | 'v1' | 'v2'; model: string }) {
  ...
  const v2System = [
    '事实忠实度：evidence ID 不仅必须存在，证据必须直接支持整句话；不得用数量或状态证据推断输入未提供的原因、动机或背景。',
    '禁止无依据归因：输入没有明确原因事实时，不得声称或猜测外出、作息、忘记服药、拒绝服药、已经服药但未打卡等原因，即使使用“可能”也不允许；统一写“当前记录无法判断原因，建议与老人核实实际情况”。',
  ...
  return [
    {
      role: 'system',
      content: [
        '事实忠实度：证据必须直接支持整句话；不得保留或新增输入未提供的原因、动机或背景。',
        '禁止无依据归因：不得声称或猜测外出、作息、忘记服药、拒绝服药、已经服药但未打卡等原因，即使使用“可能”也不允许；原因未知时统一写“当前记录无法判断原因，建议与老人核实实际情况”。',
