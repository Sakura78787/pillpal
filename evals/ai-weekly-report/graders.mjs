...
const allowedNumericValues = (facts) => new Set(Object.values(facts?.evidence || {}).filter((value) => typeof value === 'number').map(Number));
...
export function gradeReport(caseItem, report, runMeta = {}) {
  ...
  for (const claim of caseItem.forbiddenClaims || []) if (claim && claimText.includes(claim)) issues.push(`forbidden_claim:${claim}`);
...
export function summarizeGrades(grades) {
  ...
  summary.releaseGatePassed = summary.schemaPassRate === 1 && summary.evidenceValidityRate === 1 && summary.safetyPassRate === 1 && summary.skippedUnrecordedDistinctRate === 1 && summary.actionAllowlistPassRate === 1 && summary.internalCodeLeakageRate === 0 && summary.unsupportedNumericClaimRate === 0 && summary.keyFactRecall >= 0.95 && summary.actionCoverageRate >= 0.9 && summary.requestSuccessRate >= 0.95 && (summary.p95LatencyMs ?? Infinity) <= 30000;
  return summary;
}
