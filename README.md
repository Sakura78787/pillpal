...
PillPal 是一个面向慢病家庭照护场景的 Web MVP。已实现的产品链路覆盖邮箱 Magic Link 登录、当前标签页内的访客体验、用药计划、服药打卡、库存、血压/血糖/体重记录、复诊日程、同账号只读照护视角，以及基于最近 7 天记录生成的异步 AI 家庭照护周报。
...
| 已实现 | `/care` 同账号只读“子女照护视角”与访客合成照护视图、待确认 actions、AI `from=care` 衔接 | 登录用户读取本人云端数据；访客只读本标签页合成数据；均不是家庭成员绑定或跨账户共享 |
| 已实现 | 登录页访客体验、完整核心记录链路与会话内合成数据 | 数据仅在当前标签页 `sessionStorage` 保留，不上传云端、不迁移到账号；账号型页面仍需登录 |
...
> 生产站点包含本作品集的当前访客体验与同账号照护 Demo；它们不代表真实家庭账号绑定、跨账户共享或远程代操作已上线。
...
- 登录页“访客体验（无需登录）”：可使用合成数据完成核心用药、打卡、库存、健康、复诊和照护链路；数据仅在当前标签页会话内保留，刷新可恢复，关闭标签页后不再保留。
- 访客不开放数据导出、设置、提醒和 Eval Lab；访客周报仅展示固定合成 V2 示例，不读取访客操作、不调用模型或创建 AI job。
- 用药计划、已服用/已跳过记录、库存扣减与低库存提示。
- 健康页支持最近健康记录 JSON 导出；设置页通用导入/导出未启用。
- 复诊日程和应用内提示。
- `/care` 的双重体验边界：登录用户使用当前账号自己的云端数据模拟异地子女只读视角；访客使用本标签页内的合成数据。两者均不提供编辑、代打卡或跨账户读取。
