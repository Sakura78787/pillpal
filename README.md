...
PillPal 是一个面向慢病家庭照护场景的 Web MVP。已实现的产品链路覆盖邮箱 Magic Link 登录、用药计划、服药打卡、库存、血压/血糖/体重记录、复诊日程，以及基于最近 7 天记录生成的异步 AI 家庭照护周报。
...
| 当前计划 | 在同一登录账户内增加“子女照护视角”演示 | 只读模拟视角，不是家庭成员绑定或跨账户共享 |
...
稳定基线 `be4e6ff5b5b2983adc4876c8c646db4b22054856` 已验证：`npm test` 28 个测试文件 / 135 个测试通过，`npm run test:ai` 18 个测试文件 / 66 个测试通过，`npm run build` 成功（保留既有 chunk-size warning）。
